...
import { CreateRecipeInput, PatchRecipeInput, Recipe } from '../../app/api/graphql/graphql-server/modules/types.generated'
...
export const verifyPatchedRecipe = (patchedRecipe: Recipe, original: Recipe, patch: PatchRecipeInput) => {
  ...
  const sortedGroups = patchedRecipe.instructionGroups?.sort(sortById) ?? []
  const sortedExpectedGroups = expectedInstructionGroups.sort(sortById)
  if (patch.instructionGroups) {
    for (const [i, group] of sortedGroups?.entries() ?? []) {
      const expectedGroup = sortedExpectedGroups[i]
      expect(group.id).toBe(expectedGroup.id)
      expect(group.title).toBe(expectedGroup.title)
      const sortedInstructions = group.instructions?.sort(sortById) ?? []
      const expectedSortedInstructions = expectedGroup.instructions?.sort(sortById) ?? []

      for (let j = 0; j < sortedInstructions.length; j++) {
        const instruction = sortedInstructions[j]
        const expectedInstruction = expectedSortedInstructions[j]
        if (expectedInstruction.id) expect(instruction.id).toBe(expectedInstruction.id)
...
const sortById = (a: object, b: object) => (a as { id: number }).id - (b as { id: number }).id
